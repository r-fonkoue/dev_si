...
BEGIN
    if inserting or updating then
        DELETE FROM CLASSER_UTILISATEUR_EPREUVE WHERE ID_EPREUVE=:NEW.ID_EPREUVE;
        /*
        INSERT INTO CLASSER_UTILISATEUR_EPREUVE(ID_UTILISATEUR, ID_EPREUVE, POSITION) VALUES (:NEW.ID_UTILISATEUR, :NEW.ID_EPREUVE);
        */
        for POSITION in (SELECT TEMPS FROM CHRONOMETRER WHERE ID_EPREUVE=:NEW.ID_EPREUVE) loop
            INSERT INTO CLASSER_UTILISATEUR_EPREUVE VALUES (1, 1, 2);
        end loop;
        commit;
...
create or replace procedure CREER_EQUIPE(nom in varchar)
is 
ID_EQUIP EQUIPE.ID_EQUIPE%type;
BEGIN
    Select max(ID_EQUIPE) into ID_EQUIP  from EQUIPE ;
/*dbms_output.put_line('var1 : ',ID_EQUIPE);*/
    INSERT INTO EQUIPE(ID_EQUIPE,NOM,CODE,VALIDE) values (null,nom,null,null);
    COMMIT;
END;
/

create or replace procedure ENREGISTRER_TEMPS(temps in varchar)
is 
begin
     INSERT INTO CHRONOMETRER(ID_UTILISATEUR, ID_EPREUVE, TEMPS) VALUES (NULL, NULL, temps);
end;
/
...
execute AJOUT_PARTICIPATION_EQUIPE(1, 1);
